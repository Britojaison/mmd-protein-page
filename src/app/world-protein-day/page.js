...
export default function WorldProteinDay() {
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (!saveResponse.ok) {
        let errorMsg = 'Failed to save user data';
        try {
          const errData = await saveResponse.json();
          if (errData.error) errorMsg = errData.error;
        } catch (e) { }
        throw new Error(`Data Save Error: ${errorMsg}`);
      }
                      ...
                      <div className="flex flex-wrap gap-2 mb-6 lg:mb-8 justify-center">
                        {result.weeklyPlan.map((dayPlan, index) => (
                          <button
                            ...
                            className={`w-[100px] lg:w-[129px] h-[35px] lg:h-[40px] rounded-[12px] transition-all duration-300 flex items-center justify-between px-2 lg:px-4 bg-white ${selectedDay === dayPlan.day
                          ...
                          >
                            ...
                            <div className={`w-[14px] h-[14px] lg:w-[18px] lg:h-[18px] rounded-full ${selectedDay === dayPlan.day ? 'bg-gradient-to-r from-[#211E57] to-[#106D6B]' : 'border-2 border-gray-300'
                              }`}>
                            </div>
                          </button>
                        ))}
